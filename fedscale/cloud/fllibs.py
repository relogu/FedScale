...
def init_model():
    ...
    if parser.args.task == "nlp":
        ...
        model = AlbertForMaskedLM.from_pretrained(parser.args.model)
        tokenizer = AlbertTokenizer.from_pretrained(
    ...
    # Add leaf shakespeare model
        ...
        if parser.args.model == "albert":
            ...
            config = AutoConfig.from_pretrained(
                os.path.join(parser.args.log_path, "albert-small-config.json")
            )
        ...
        model = LogisticRegression(
            parser.args.vocab_token_size, parser.args.vocab_tag_size
        )
    elif parser.args.task == "speech":
        ...
        audio_conf = dict(
            sample_rate=parser.args.sample_rate,
        ...
        model = DeepSpeech(
            rnn_hidden_size=parser.args.hidden_size,
        ...
        model = resnet(
            readClass(os.path.join(parser.args.data_dir, "class.txt")),
            50,
            pretrained=True,
            class_agnostic=False,
            pretrained_model=parser.args.backbone,
        )
    ...
    if parser.args.task == "detection":
        if not os.path.exists(parser.args.data_cache):
            ...
            imdb, roidb, ratio_list, ratio_index = combined_roidb(
                ...
                sizes=parser.args.train_size_file,
            )
            train_dataset = roibatchLoader(
                roidb,
                ratio_list,
                ratio_index,
                parser.args.batch_size,
                imdb.num_classes,
            ...
            imdb_, roidb_, ratio_list_, ratio_index_ = combined_roidb(
                imdbval_name,
                ["DATA_DIR", parser.args.data_dir],
                sizes=parser.args.test_size_file,
                training=False,
            )
        ...
        # Add leaf shakespeare dataset
            ...
            if parser.args.model == "albert":
                ...
                train_dataset = fl_loader.AmazonReview_loader(
                    ...
                    train=True,
                    tokenizer=tokenizer,
                    max_len=parser.args.clf_block_size,
                )
