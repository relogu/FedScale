...
class TorchClient(ClientBase):
    ...
    def train(self, client_data, model, conf):
        ...
        if os.path.exists(filename):
            with open(filename, "a") as out:
                out.write(
                    f"{client_id},{t1-t0},{len(client_data.dataset.index)},{int(len(client_data.dataset.index)/20)}\n"
                )
        else:
            with open(filename, "x") as out:
                out.write(
                    f"client_id,training_time,n_samples,n_batches\n{client_id},{t1-t0},{len(client_data.dataset.index)},{int(len(client_data.dataset.index)/20)}\n"
                )
        ...
        results["utility"] = math.sqrt(self.loss_squared) * float(
    ...
    def train_step(self, client_data, conf, model, optimizer, criterion):
        ...
        for i, data_pair in enumerate(client_data):
            ...
            if conf.task == "detection":
                ...
                (data, masks) = data
                data, masks = Variable(data).to(device=self.device), Variable(masks).to(
                    device=self.device
                )
            ...
            if conf.task == "nlp" or (
                conf.task == "text_clf" and conf.model == "albert-base-v2"
            ):
            ...
            self.optimizer.update_client_weight(
                conf,
                model,
                self.global_model if self.global_model is not None else None,
            )
    ...
    def test(self, client_data, model, conf):
        ...
        logging.info(
            ...
            "test_5_accuracy {:.2f}% \n".format(
                round(time.time() - evalStart, 4), test_loss, acc * 100.0, acc_5 * 100.0
            )
        )
        return test_results
