...
def process_cmd(yaml_file, local=False):
    ...
    executor_configs = (
        "=".join(yaml_conf["worker_ips"]) if "worker_ips" in yaml_conf else ""
    )
    ...
    #     monitor_process = subprocess.Popen(monitor_cmd, shell=True, stdout=fout, stderr=fout)
    ...
    with open(f"{job_name}_logging", "a") as fout:
        if local:
            local_process = subprocess.Popen(
                f"{ps_cmd}", shell=True, stdout=fout, stderr=fout
            )
            local_pid = local_process.pid
            print(
                f"Aggregator local PID {local_pid}. Run kill -9 {local_pid} to kill the job."
            )
        else:
            subprocess.Popen(
...
def submit_to_k8s(yaml_conf):
    ...
    for name, meta_dict in k8s_dict.items():
        if meta_dict["type"] == "aggregator":
            ...
            if aggr_ip == -1:
                print(
                    f"Error: aggregator {name} not ready after maximum waiting time allowed, aborting..."
                )
                exit(1)
